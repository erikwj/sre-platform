    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          runbookProcedures: data.runbookProcedures || '',
        });
        setMonitoringLinks(data.monitoringLinks || []);
        setUpstreamServices(data.upstreamServices || []);
        setDownstreamServices(data.downstreamServices || []);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Failed to update runbook');
      }
    } catch (err) {
      setError('Failed to update runbook. Please try again.');
      console.error('Error updating runbook:', err);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-8 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Runbook</h1>
